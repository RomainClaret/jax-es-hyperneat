"""
HSHG ES-HyperNEAT XOR experiment.
Uses HSHG-based ES-HyperNEAT reimplementation with TensorNEAT for XOR problem.

Identical to the tensorneat_xor experiment except substrate discovery uses
HSHG (Hierarchical Spatial Hash Grid) instead of quadtree.
"""
...
@dataclass
class XORConfig:
    """Configuration for XOR ES-HyperNEAT experiment matching PUREPLES."""
    # NEAT parameters for evolving CPPN (matching PUREPLES config_cppn_xor)
    pop_size: int = 150
    species_size: int = 15
...
class XORProblem(problem.FuncFit):
    """XOR problem implementation with bias input to match PUREPLES."""
    ...
    @property
    def inputs(self):
        # XOR inputs with bias column to match PUREPLES
        return np.array(
    ...
    def input_shape(self):
        return 4, 3  # 4 samples, 3 inputs (x1, x2, bias)

    @property
    def output_shape(self):
        return 4, 1  # 4 samples, 1 output


class HSHGESHyperNEATExperiment:
    """HSHG ES-HyperNEAT experiment for XOR."""

    def __init__(self, config: Optional[XORConfig] = None,
    ...
    def run(self):
        ...
        for generation in range(self.config.max_generations):
            ...
            # Clear JAX traces and run garbage collection to prevent memory leak
            import gc
            gc.collect()
            jax.clear_caches()
