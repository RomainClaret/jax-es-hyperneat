"""
HSHG ES-HyperNEAT Sine regression experiment.
Uses HSHG-based ES-HyperNEAT reimplementation with TensorNEAT for Sine problem.

Sine regression: 1 input x in [-1, 1], output = sin(pi * x).
20 evenly spaced training points.
Fitness = -MSE (TensorNEAT convention: higher is better, so negative MSE).

Identical to tensorneat_sine except substrate discovery uses HSHG instead of quadtree.
"""
...
def generate_sine_data(n_points: int = 20):
    """Generate sine regression dataset.

    Args:
        n_points: Number of evenly spaced points in [-1, 1].

    Returns:
        Tuple of (inputs_array, targets_array) as numpy arrays
        with bias column appended to inputs.
    """
    x_vals = np.linspace(-1.0, 1.0, n_points).astype(np.float32)
    y_vals = np.sin(np.pi * x_vals).astype(np.float32)
    # Append bias column
    bias = np.ones((n_points, 1), dtype=np.float32)
    inputs = np.hstack([x_vals.reshape(-1, 1), bias])
    targets = y_vals.reshape(-1, 1)
    return inputs, targets


@dataclass
class SineConfig:
    """Configuration for Sine ES-HyperNEAT experiment matching PUREPLES."""
    ...
    fitness_target: float = -0.05  # TensorNEAT uses negative MSE
    seed: int = 42

    # Substrate coordinates matching PUREPLES
    # One input (x) + bias
    input_coors: Tuple[Tuple[float, float], ...] = ((-1.0, -1.0), (1.0, -1.0))
...
class SineProblem(problem.FuncFit):
    """Sine regression problem with bias input to match PUREPLES."""

    def __init__(self):
        super().__init__(error_method="mse")
        self._inputs, self._targets = generate_sine_data()
    ...
    def input_shape(self):
        return self._inputs.shape  # (20, 2): x, bias

    @property
    def output_shape(self):
        return self._targets.shape  # (20, 1)


class HSHGESHyperNEATExperiment:
    """HSHG ES-HyperNEAT experiment for Sine regression."""

    def __init__(self, config: Optional[SineConfig] = None,
        ...
        """
        self.config = config or SineConfig()
    ...
    def run(self):
        ...
        print(f"\nRunning HSHG ES-HyperNEAT Sine regression...")
        ...
        for generation in range(self.config.max_generations):
            ...
            # Check for early termination (fitness target 0.95 for sine)
